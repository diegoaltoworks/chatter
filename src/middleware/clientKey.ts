 ...
 * fake value on every request and bypass per-IP limits entirely, so
 * `trustProxy` defaults to `false`: pass `trustProxy: true` only when such a
 * proxy is guaranteed to be in front. Left `false`, every caller collapses
 * onto one shared bucket, which is coarse but not bypassable — pair it with
 * a fronting proxy that does the real per-client limiting.
 */
